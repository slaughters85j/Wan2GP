...
class SigmaAwareGatePerTokenPerDim(nn.Module):
    ...
    def compute_gate_scalar(
        ...
        sigma_offset = -self.log_alpha.exp().to(content_logit.dtype) * sigma.to(dtype=content_logit.dtype).view(-1, 1, 1)  # (B, 1, 1)
        return content_logit.add_(sigma_offset).sigmoid_()  # (B, N, D)
