...
from postprocessing.pid.networks.lq_projection_2d import LQProjection2D
from postprocessing.pid.networks.pixeldit_official import PixDiT_T2I, _take_tensor
from postprocessing.pid.utils.context_parallel import cat_outputs_cp_with_grad, split_inputs_cp
...
class PidNet(PixDiT_T2I):
    ...
    @staticmethod
    def _add_silu(target, other):
        target.add_(other)
        return torch.nn.functional.silu(target, inplace=True)
    ...
    def _compute_lq_features(self, lq_video_or_image, lq_latent, lq_mask, Hs, Ws):
        ...
        if lq_mask is not None:
            mask = lq_mask.view(-1, 1, 1)
            for feature in lq_features:
                feature.mul_(mask)
        ...
        for i in range(self.patch_depth):
            if has_lq and self.lq_proj.is_gate_active(i):
                ...
                if out_idx < len(lq_features):
                    lq_feature = lq_features[out_idx]
                    if lq_feature is not None:
                        lq_features[out_idx] = None
                        lq_feature_list = [lq_feature]
                        lq_feature = None
                        s_main_list = [s_main]
                        s_main = None
                        s_main = self.lq_proj.gate(s_main_list, lq_feature_list, sigma=degrade_sigma, out_idx=out_idx)

            s_main_list = [s_main]
            y_emb_list = [y_emb]
            s_main = None
            y_emb = None
            s_main, y_emb = self.patch_blocks[i](s_main_list, y_emb_list, condition, pos, pos_txt, attn_mask_joint)
            ...
            if torch.is_grad_enabled() and 0 < self.repa_encoder_index == (i + 1):
    ...
    def forward(
        ...
        pad_h = (-H_orig) % self.patch_size
        pad_w = (-W_orig) % self.patch_size
        if pad_h or pad_w:
        ...
        y = None
        y_pos = self.y_pos_embedding[:, :Ltxt, :].to(y_emb.dtype)
        y_emb.add_(y_pos)
        del y_pos
        ...
        if s is None:
            ...
            if self.use_ed and self.encoder_ed is not None and self.decoder_ed is not None:
                ...
                s_main = self._add_silu(s_main, t_emb)
                ...
                s_main, y_emb, collected_features = self._run_patch_blocks(
                    s_main_list, y_emb, condition, pos_b, pos_txt, attn_mask_joint, lq_features, degrade_sigma=degrade_sigma, feature_indices=feature_indices
                )
                ...
                s_main = None
                s_main, y_emb, collected_features = self._run_patch_blocks(
                    s_main_list, y_emb, condition, pos, pos_txt, attn_mask_joint, lq_features, degrade_sigma=degrade_sigma, feature_indices=feature_indices
                )

                s = self._add_silu(s_main, t_emb)
                s_main = None

            y_emb = None
            t_emb = None
            condition = None
            if not self.pit_lq_inject:
                lq_features = None

        if torch.is_grad_enabled() and not (0 < self.repa_encoder_index <= self.patch_depth):
