...
def exec_prompt_enhancer_engine(state, model_type, model_def, prompt_enhancer_modes, original_prompts, image_start, original_image_refs, is_image, audio_only, seed, progress, override_profile, send_cmd = None, tools = None, enhancer_kwargs = None, alt_prompts = None, alt_prompt_inherits_prompt_paragraphs = False):
    ...
    # The Deepy remote backend answers enhancement over HTTP (process_prompt_enhancer's own
    # remote guard does the work), so it counts as non-local and skips loading the local model.
    local_runtime = not is_remote_engine(resolve_role_engine(server_config, "prompt_enhancer")) and not deepy_remote_runtime.remote_backend_active()
    if local_runtime:
        wait_for_model_unload()
